...
# Original Game : Robin Drink by David !
# Modified Game : Robin Strip by LaDite
...
NbSegmentsForPlayer = 7     # nombre de segments pour faire le joueur courant
...
class Game(cgame.Game):
   ...
   def pre_dart_check(self,players,actual_round,actual_player,player_launch):
        ...
        if actual_round == 1 and player_launch == 1 and actual_player == 0 and self.firstStart:
            ...
            #change background
            self.display.display_background('bg_robin_strip')
        ...
        if player_launch == 1:
            ...
            for i in range (0, NbSegmentsForOpponents):
                ...
                segments[f"S{hits[h]}"] = 'green'
                segments[f"D{hits[h]}"] = 'green'
                segments[f"T{hits[h]}"] = 'green'
                hits.pop(h)
            ...
            for i in range (0, NbSegmentsForRandom):
                ...
                segments[f"S{hits[h]}"] = 'blue'
                segments[f"D{hits[h]}"] = 'blue'
                segments[f"T{hits[h]}"] = 'blue'
                hits.pop(h)
        ...
        if player_launch == 2 :
            ...
            for i,p in enumerate(players) :
                for sft in segmentsForTarget[i][0] :
                    segments["f'S{sft}"] = segmentsForTarget[i][1]
                    segments["f'D{sft}"] = segmentsForTarget[i][1]
                    segments["f'T{sft}"] = segmentsForTarget[i][1]
...
##### **** Pour eviter de retirer trop de vetement en une fois, mettre le multiplicateur (multi) a 1 pour DOUBLE et TRIPLE
...
##### **** Pour afficher les messages (showmessage), les modifs se font dans LOCALE
   ...
   def refresh_game_screen(self, players, actual_round, max_round, RemDarts, nb_darts, logo, headers, actual_player,TxtOnLogo=False, Wait=False, OnScreenButtons=None, showScores=True, end_of_game=False, endOfSet=None, Set=None, MaxSet=None):
      ...
      if not end_of_game:
          ...
          self.display.display_image(self.display.file_class.get_full_filename('robin_strip/robin_strip_target', 'images'),rect[0], rect[1], rect[2], rect[3], True, False, False)

          if self.player_launch == 1:
              for s in self.segmentsOpponents:
                self.display.display_image(self.display.file_class.get_full_filename(f'robin_strip/G{s}', 'images'), rect[0], rect[1], rect[2], rect[3], True, False, False)
              for s in self.segmentsRamdom:
                self.display.display_image(self.display.file_class.get_full_filename(f'robin_strip/B{s}', 'images'), rect[0], rect[1], rect[2], rect[3], True, False, False)
              for s in self.segmentsPlayer:
                self.display.display_image(self.display.file_class.get_full_filename(f'robin_strip/R{s}', 'images'), rect[0], rect[1], rect[2], rect[3], True, False, False)
          else:
              for player in players:
                for s in segmentsForTarget[player.ident][0]:
                   self.display.display_image(self.display.file_class.get_full_filename(f'robin_strip/{segmentsForTarget[player.ident][2]}{s}', 'images'),rect[0], rect[1], rect[2], rect[3], True, False, False)

          for player in players :
            selected = (1 if player.ident == actual_player else 0)
            self.show_player_name(player, Colors.Colors.get(segmentsForTarget[player.ident][1], 'black'), selected)
   ...
   def random_opponent(self, players, actual_player) :
      ...
      for i in range(0, nb - 1):
          ...
          rect = ((self.display.res['x'] - self.targetH) / 2, (self.display.res['y'] - self.targetH) / 2, self.targetH, self.targetH)

          self.display.display_image(self.display.file_class.get_full_filename('robin_strip/robin_strip_target', 'images'), rect[0], rect[1], rect[2], rect[3], True, False, False)
          self.display.display_image(self.display.file_class.get_full_filename(f'robin_strip/R{self.target_order[seg % 20]}', 'images'), rect[0], rect[1], rect[2], rect[3], True, False, False)

          self.display.update_screen(rect=(rect[0], rect[1], rect[2], rect[3]))
   ...
   def show_player_name(self, player, color, selected) :
       ...
       self.display.display_image(self.display.file_class.get_full_filename(f'robin_strip/robin_player_sel{selected}', 'images'),self.pos[player.ident][0], self.pos[player.ident][1], self.bw, self.bh, True, False, False)
   ...
   def show_message(self, typeMessage, player, multi, delay=4000, refreshbackground=True) :
      ...
      self.display.display_image(self.display.file_class.get_full_filename('robin_strip/robin_strip_box', 'images'), x, y - h / 3, w, h + h * 2 / 3, True, False, False)

      self.display.display_image(self.display.file_class.get_full_filename(f'robin_strip/robin_strip_img_{numImg}', 'images'), x - h / 3, y + h / 2, h, h * 2, True, False, False)
